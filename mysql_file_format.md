...
上述是使用 `shell` 实现的一个程序，其实现了基本的数据库的写入、读取的功能。虽然其阐释了一个本质：数据库无非是一个可以将数据写入文件持久化保存，并支持从文件中读取之前已保存的数据的程序。但由于其简单的文件组织形式，使得其远远不能胜任日常使用。
...
MySQL InnoDB 存储引擎中的文件众多，如果只是单纯的介绍各种类型的文件，极有可能如背八股文一般，既枯燥又让人莫名其妙。下面我们将尝试自己设计一个支持 CRUD 的数据库文件，以窥探 MySQL InnoDB 存储引擎设计者的良苦用心。

如果让你来设计一种文件格式，支持对符合 SQL 规范的表进行 CRUD（比如下面的 `record_format_demo` 表），你会如何开始？
``` sql
...
首先，放轻松，这不是面试。即使设计的十分简单，但只要能运行，这就算是一个及格的程序。

### 行记录

如同上所述 shell 程序使用的 `key-value` 格式，如果需要支持多个列的记录格式，我们第一想法就是多个列数据从左至右依次记录：
```
...
- 各个列数据之间使用符号隔开，比如逗号（同 csv 格式一样）。但这有个问题，用户可能会写入逗号，与分隔符一致，从而导致数据读取错误
- 各个列数据都以 4 * N 个字节存储，如果不足 4 * N 个字节，则左侧使用 0 进行填充。但这会造成大量的空间浪费（即使存储 1 个字节的字符数据也需要至少 4 个字节空间）
- 在每条记录数据空间之外使用额外空间存储字段长度（如下图）。首先我们读取该数据获取字段长度，然后分别读取对应区间的数据，即可获取对应字段值。
```
INSERT INTO record_format_demo(c1, c2, c3, c4) VALUES('aaaa', 'bbb', 'cc', 'd')
...
假设一个字段的定义为 `varchar(50)`，则其字段长度区间为 0~50，我们只需要 1 个字节就可以记录这个区间。记作 `0x32`。
假设一个字段的定义为 `varchar(65535)`，则其字段长度区间为 0~65535，这个时候需要 1 或 2 个字节来记录。记作 `0xffff`。

##### 如何区分字段长度数据？

现在有一个 `0x32ffff` ，这是应该解读为 `0x32ff`、`0xff` 长度，还是 `0x32`、`0xffff`呢？

第一种方式，计算出所有字段值长度记录所需的最大字节数，然后每个字段值长度都以这个长度的空间进行记录。比如最长的字段长度需要 2 个字节来记录，则每个字段长度都使用两个字节来记录（不足的空间左侧使用 0 填充）。则 `0x32` 、`0xffff` 应该即为 `0x0032`、`0xffff`。

第二种方式，每个字段长度使用的字节数按需分配，字段长度小的使用 1 个字节存储，大的使用 2 个字节存储。每个字节的第一个二进制位是否为 0 来标识当前字节是否是完整的字段长度。则 `0x32` 、`0xffff` 应该即为 `0x32`、`0x03ffff`（0000 0011 1111 1111 1111 1111）。

#### NULL 值列表

由于字段支持 NULL 值，如果将 NULL 值以特殊字符（如：`0x78857676`，即 NULL 字符）存储在数据空间，则会出现以下一些问题：
- 可能与用户数据 `NULL` 混淆
- 破坏了变长字段长度列表设计，是应该记录该字段长度为 4 还是 0 呢？
- 空间利用率差，使用 4 个字节来表示 NULL ，只需一个比特标识位记录即可

既然在数据内容中存储 NULL 不行，那就申请单独的一块空间用于记录可以为 NULL 的字段值是否为 NULL。
```
...
由于 c1、c3、c4 字段可能为 NULL，所以 NULL 值列表只有 3 个比特位用以表示对应 3 个字段的 NULL 值情况。为 0 代表非 NULL，为 1 代表为 NULL。
...
还有另一种办法，即先将该记录标记为已删除，稍后通过后台线程批量将已被删除的记录的物理空间进行释放，并调整其他有效记录位置。这种方法可以有效降低单次删除操作所需的时间。

为此我们需要一个额外空间来记录当前记录是否被删除，我们可以使用单独的空间或者是在记录头上使用一个二进制位用来记录删除状态，为了方便维护和实现，我们选择后者。
...
- 首先，我们创建一个文件（叫为`数据页`），记录在文件中一条一条的紧密排列着，从而保证空间的有效利用
- 其次，数据页中的记录应该按照顺序进行排序，从而为后续查询优化提供基础（如：最小、大值比对，二分查找）

#### 记录之间的链接

通过上述记录格式，我们可以计算出一条记录的总共长度，从而实现页内记录之间的区分。但由于记录存在被删除的情况，所以下一条记录并不一定是有效的，通过当前记录找到下一条有效记录需要通过不断遍历文件内容实现。

我们在记录头上增加一个元数据（2字节），存储下一条有效记录的偏移量，就可以快速找到下一条有效记录。
```
...
('eeee', 'fff', NULL, NULL);
                                          ...
                                          下条记录的偏移量
                                               ↓ 
----------------------------------------------------------------
| c1.len | c2.len | c4.len  | 0 | 0 | 0 | 0 | 0x24 |c1|c2|c3|c4| ---|
----------------------------------------------------------------    |
																    |
--------------------------------------------------                  |
| c1.len | c2.len | 0 | 1 | 1 | 0 | 0 | c1 | c2 |    <--------------|
--------------------------------------------------
                                    ↑
                               下条记录的偏移量
```
...
即使页内记录组成了一条单向链表，但有时候查找对应记录仍需要遍历整个文件。既然记录之间有序，通过二分查找来优化查询问题变得理所当然。

对于单链表的二分查找，我们需要一个额外的空间记录链表中的关键节点值，这个额外空间为 slots，其中存储了若干个 slot，每个 slot 指向对应的接口。

```
查找 5
1，对 slots 进行二分查找，直到找到合适的 slot
2，找到 slot 指向的记录
3，按照记录链表迭代查找记录，直到找到目标记录

注意：由于第 3 步会按照链表迭代查找记录，所以我们应知道迭代的终止条件，以避免无效查询。使用一个元数据记录该 slot 对应记录的条数，放在 slot 指向的第一条记录头上。
...
我们将这个 slots 与记录空间区分开，作为数据页的单独一部分。

#### 提升多个页记录的查询速度

前提：不仅是页内数据是按照顺序排序的，页与页之间的数据也是按照顺序排序的。

使用额外的空间记录各个数据页的最小记录值，当需要查询对应记录时，先到该页查找对应的数据页，然后再用上述方法在数据页中查找到对应的记录。这个额外的空间即为`索引页`。
