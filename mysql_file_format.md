...
现在有一个 `0x32ffff` ，这是应该解读为 `0x32ff`、`0xff` 长度，还是 `0x32`、`0xff`呢？
...
第二种方式，每个字段长度使用的字节数按需分配，字段长度小的使用 1 个字节存储，大的使用 2 个字节存储。每个字节的第一个二进制位是否为 0 来标识当前字节是否是完整的字段长度。则 `0x32` 、`0xffff` 应该即为 `0x0032`、`0x03ffff`（0000 0011 1111 1111 1111 1111）。
...
还有另一种办法，即先将该记录标记为已删除，稍后通过后台线程批量将已被删除的记录的物理空间进行擦除，并调整其他有效记录位置。这种方法可以有效降低单次删除操作所需的时间。
